...
def search_query_against_db(query_dict, target_dict, mincov, topk, score_corrections=None):
    ...
    scores = F.cosine_similarity(target_dict['database'],
...
def dbsearch(query, target_dict: dict, tmp: str, network: FoldClassNet,
    ...
    with torch.no_grad():
        ...
        query_input = torch.from_numpy(query_dict['coords'])\
            .unsqueeze(0).to(device)
        query_dict['embedding'] = network(query_input)

        result_dict = search_query_against_db(
            query_dict=query_dict, target_dict=target_dict, mincov=mincov,
            topk=topk, score_corrections=score_corrections)
...
def dbsearch_faiss(queries: list[dict], target_dict: dict, tmp: str, network: FoldClassNet,
    ...
    if pdb_chain:
        ...
        if len(pdb_chains) != nq:
            logger.error("Number of supplied chain IDs doesn't match number of queries.")
            logger.error("%d queries and %d chain IDs." % (nq, len(pdb_chains)))
            sys.exit(1)
    # else:
    #     pdb_chains = ["A"] * nq
    ...
    # TODO the TM-aligns can (and should) be run in parallel
...
def run_dbsearch(inputs: list[str], db_name: str, tmp: str, device: torch.device, topk: int, fastmode: bool,
    ...
    if target_db['faiss']:
        ...
        # if pdb_chain:
        # in `search` there should only be one chainid or `len(inputs)` chainids
        # chainid also defaults to 'A' so is never None
        pdb_chain = pdb_chain.rstrip(",")
        pdb_chains = pdb_chain.split(",")
